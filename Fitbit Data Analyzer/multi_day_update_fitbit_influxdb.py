...
FITBIT_ACCESS_TOKEN = '' # Use this for single testing. or use the .fitbit-refreshtoken method. 
FITBIT_INITIAL_CODE = ''
...
filehandle = open("/home/arpan/.database_update_log", "a")
...
if not FITBIT_ACCESS_TOKEN:
    if os.path.isfile('/home/arpan/scripts/python_scripts/.fitbit-refreshtoken'):
        f = open('/home/arpan/scripts/python_scripts/.fitbit-refreshtoken', "r")
        token = f.read().rstrip()
        f.close()
        response = requests.post('https://api.fitbit.com/oauth2/token',
                    data={
                        ...
                        "refresh_token": token
                    }, headers={'Authorization': 'Basic ' + '**<base64encode(client_id + ":" + client_secret)>**', 'Content-Type': "application/x-www-form-urlencoded"}) # Server apps must pass an Authorization header with their Basic Token, which is their Client ID and Secret formatted like so: "Basic " + base64encode(client_id + ":" + client_secret).
    else: # This method of getting refresh token is outdated., Please generate your first refresh token manually and put in the .fitbit-refreshtoken file. 
        # response = requests.post('https://api.fitbit.com/oauth2/token',
        #     data={
        #         "client_id": FITBIT_CLIENT_ID,
        ...
        print("This method of getting refresh token is outdated., Please generate your first refresh token manually and put in the .fitbit-refreshtoken file.")
