...
def canstay(l,pos,num):
    ...
    if rem==0:
        ...
        horstart=(pos-rem)
    horange=list(range(horstart,horstart+9))
    ...
    for subbox in mainbox:
        if pos in subbox:
            ...
            break
    if rem==0:
        varrange=list(range(0,81,9))
    else:
        varrange=list(range(rem,81,9))
    horange.remove(pos)
    varrange.remove(pos)
...
def replaceline(l,flag):
    for index in range(0,9):
        rowlist = list(range(index,81,9))
        l = fill(l,rowlist)
        if flag == 'hard':
            l = rangelogic(l,rowlist)
    for index in range(0,81,9):
        columnlist = list(range(index,index+9))
        l = fill(l,columnlist)
        if flag == 'hard':
    ...
    for index in range(81):
        if l[index] == '-':
            ...
            if len(possible) == 0:
                ...
                x = input("Wrong values found!! Press ENTER to EXIT!>>>")
                exit()
    return l
...
def takeinput():
    ...
    while len(inputlist)<81:
        try:
            inp = input("Enter the value number "+str(len(inputlist)+1)+" : ")
            if inp == 'x':
                inputlist.append('-')
...
print("GoodBye!!")
x = input("Press  Enter to exit:")#pauses the script from closing
