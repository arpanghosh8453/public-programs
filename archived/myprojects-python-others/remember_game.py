...
import string
def randomnum(length=10, chars=string.letters[0:54]+string.digits):
    from random import choice
    import string
...
def main(level):
    ...
    if level<3 and level>0:
        ...
        char = string.letters[0:26]                   
    elif level<7 and level>4:
        char = string.letters[0:52]
    elif level<9 and level>6:
        char = string.letters[0:52]+string.digits
    else:
        char = string.letters[0:52]+string.digits+'#$%&\()*+-/:;<=>?@[\\]^_!~'
